...
public class PowerGeneratorService {
	...
	/*@GET
	@Path("/getPowerGenerator") 
	@Produces(MediaType.TEXT_HTML) 
	public String getPowerGenerator(String powerGeneratorData) 
	{ 
		//Convert the input string to an XML document
		 Document doc = Jsoup.parse(powerGeneratorData, "", Parser.xmlParser()); 
		 
		//Read the value from the element <itemID>
		 String gName = doc.select("gName").text(); 
		 String output = pGen.getPowerGenerator(gName); 
		return output; 
	}*/
	
	
	@PUT
	@Path("/updatePowerGenerator") 
